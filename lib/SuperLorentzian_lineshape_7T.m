...
% strength, w_loc. Includes shift of SL lineshape at 7T.
% 
% [G,w_loc] = SuperLorentzian_lineshape_7T(T2s,fsample,varargin)
% 
...
% David Leitao (c), King's College London, June 2021

function [G,w_loc] = SuperLorentzian_lineshape_7T(T2s,fsample,varargin)
...
%%% Frequency shift at 7T
ff0 = -773;

[thg,ffg]=meshgrid(th,ff);

g = sin(thg).*sqrt(2/pi).*T2s./(abs(3*cos(thg).^2-1));
g = g .* exp(-2*(2*pi*(ffg-ff0)*T2s./abs(3*cos(thg).^2-1)).^2);
